...
def main():
    ...
    output_path = f"../output/{model_name}/olympiad_output_prompt_len_{args.prompt_len}_num_pages_{num_pages if not args.use_dense_kv else 'dense'}_{args.algo}.jsonl"
    ...
    with open(output_path, "w", encoding="utf-8") as fout:
        for i in tqdm(range(0, len(ruler_data), bs)):
            ...
            # remove this prompt for Qwen2-math because it already has the prompt in the chat template
            ...
            for _ , (item, ruler_item, pred) in enumerate(zip(inputs, batch, predictions)):
                output_item = {
                    "input": item,
                    "gold_output": ruler_item["solution"],
